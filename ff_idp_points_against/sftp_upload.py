#!/usr/bin/env python3
"""
Upload every season's IDP points-against JSON to the live site over SFTP.
Same account/pattern as ff_utilization/ff_points_against's sftp_upload.py --
see either for the one-time SiteGround SFTP setup if `.env` isn't already
filled in.

This is upload-only. It does not run the fetch step -- run
fetch_nflverse_idp_points_against.py first, check the numbers, THEN upload.

Every `data/idp_points_against_<year>.json` found locally is uploaded to
`wp-content/uploads/f6p-data/idp_points_against_<year>.json` -- the same
fixed path component.html's season dropdown fetches from.
...
def connect():
    ...
    if not host or not username:
        sys.exit("Missing SFTP_HOST or SFTP_USERNAME -- copy .env.example to "
                  ...
                  "ff_points_against/.env, same account).")
...
def discover_files():
    out = {}
    for path in sorted(DATA.glob("idp_points_against_*.json")):
        m = re.match(r"idp_points_against_(\d{4})\.json$", path.name)
        if not m:
            continue
...
def main():
    ...
    if not root:
        sys.exit("Missing SFTP_REMOTE_ROOT -- see ff_points_against/"
                  "sftp_upload.py's docstring, same account, same value.")

    files = discover_files()
    if not files:
        sys.exit(f"No data/idp_points_against_<year>.json files found in "
                  f"{DATA} -- run fetch_nflverse_idp_points_against.py first.")
